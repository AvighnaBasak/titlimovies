...
export default function tmdbLoader({ src, width }) {
    ...
    // Pick one bucket up from exact match for crispness on retina/high-DPR screens
    let size = 'original';
    if (width <= 92) size = 'w154';
    else if (width <= 154) size = 'w185';
    else if (width <= 185) size = 'w342';
    else if (width <= 342) size = 'w500';
    else if (width <= 500) size = 'w780';
    // width > 500 → 'original' (hero backdrops, etc.)
