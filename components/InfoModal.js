...
// Fast ease-out curve for mobile — no spring physics overhead
// Uses Apple's "fast out" curve for responsive feel without jank
const MOBILE_EASE = { type: "tween", duration: 0.35, ease: [0.32, 0.72, 0, 1] };
...
export default function InfoModal() {
    ...
    const swipeState = useRef({ startY: 0, startScrollTop: 0, isDragging: false, offset: 0 });
    ...
    // ── Swipe-to-dismiss (mobile only, Netflix-style) ──
    // When at scroll top, pulling down dismisses the modal.
    // Uses native touch events with passive:false to preventDefault during drag.
    useEffect(() => {
        const isMob = typeof window !== 'undefined' && window.innerWidth < 768;
        if (!isMob || !isModalOpen) return;
        ...
        const onTouchStart = (e) => {
            ...
            swipeState.current.startScrollTop = scrollEl.scrollTop;
        ...
        const onTouchMove = (e) => {
            ...
            // Only activate drag if: started at scroll top AND dragging down
            if (swipeState.current.startScrollTop <= 5 && deltaY > 8) {
                swipeState.current.isDragging = true;
                swipeState.current.offset = deltaY;

                // Direct DOM manipulation for 60fps (no React re-renders)
                const progress = Math.min(deltaY / 400, 1);
                const opacity = 1 - progress * 0.4;
                const scale = 1 - progress * 0.05;
                wrapperEl.style.transform = `translate3d(0, ${deltaY}px, 0) scale(${scale})`;
                wrapperEl.style.opacity = String(opacity);
                wrapperEl.style.transition = 'none';

                // Also fade the overlay background
                scrollEl.style.backgroundColor = `rgba(0,0,0,${0.75 - progress * 0.5})`;
        ...
        const onTouchEnd = () => {
            ...
            const THRESHOLD = 120;
            if (swipeState.current.offset > THRESHOLD) {
                // Swipe far enough → animate out and close
                wrapperEl.style.transition = 'transform 0.28s cubic-bezier(0.32, 0.72, 0, 1), opacity 0.28s ease';
                wrapperEl.style.transform = 'translate3d(0, 100vh, 0) scale(0.9)';
                wrapperEl.style.opacity = '0';
                setTimeout(() => {
                    closeModal();
                    // Reset wrapper after close
                    wrapperEl.style.transition = 'none';
                    wrapperEl.style.transform = '';
                    wrapperEl.style.opacity = '';
                    scrollEl.style.backgroundColor = '';
                }, 300);
            } else {
                // Didn't reach threshold → snap back
                wrapperEl.style.transition = 'transform 0.3s cubic-bezier(0.32, 0.72, 0, 1), opacity 0.2s ease';
                wrapperEl.style.transform = 'translate3d(0, 0, 0) scale(1)';
                wrapperEl.style.opacity = '1';
                scrollEl.style.transition = 'background-color 0.3s ease';
                scrollEl.style.backgroundColor = '';
            }
    ...
    return createPortal(
        <motion.div
            ...
            className={`fixed inset-0 z-[1000] overflow-y-auto overflow-x-hidden flex justify-center items-start pt-0 md:pt-8 pb-0 md:pb-8${isMobile ? ' mobile-modal-overlay' : ''}`}
            onClick={closeModal}
            initial={{ backgroundColor: "rgba(0,0,0,0)" }}
            animate={{ backgroundColor: isMobile ? "rgba(0,0,0,0.75)" : "rgba(0,0,0,0.7)" }}
            exit={{ backgroundColor: "rgba(0,0,0,0)" }}
            transition={isMobile ? { duration: 0.25 } : { duration: 0.3 }}
            style={isMobile ? {} : { backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)" }}
        >
            {/* Swipe wrapper — transforms independently from Framer Motion */}
            <div ref={swipeWrapperRef} style={{ width: '100%', maxWidth: isMobile ? '100%' : '850px' }}>
                <motion.div
                    className={`relative w-full bg-black md:bg-[#181818] rounded-t-2xl md:rounded-xl overflow-visible md:overflow-hidden shadow-2xl mx-0 md:mx-4 mb-0 md:mb-8 pb-12 md:pb-0 min-h-screen md:min-h-auto${isMobile ? ' mobile-modal-sheet' : ''}`}
                    onClick={e => e.stopPropagation()}
                    style={{ minHeight: '80vh' }}
                    // Mobile: simple GPU-accelerated slide-up (no spring physics)
                    // Desktop: spring-based scale-in from center
                    initial={isMobile ? { y: "100%", opacity: 0.8 } : { y: 20, opacity: 0, scale: 0.9 }}
                    animate={isMobile ? { y: 0, opacity: 1 } : { y: 0, opacity: 1, scale: 1 }}
                    exit={isMobile ? { y: "100%", opacity: 0 } : { y: 20, opacity: 0, scale: 0.9 }}
                    transition={isMobile ? MOBILE_EASE : NETFLIX_SPRING}
                >
                    {/* Mobile swipe handle pill — visual hint for pull-to-dismiss */}
                    {isMobile && (
                    ...
                    {/* Close Button — h-0 sticky on mobile (no black bar), absolute on desktop */}
                    <div className="sticky top-0 z-50 h-0 overflow-visible pointer-events-none md:absolute md:top-4 md:right-4 md:h-auto md:w-auto">
                        <button
                            onClick={closeModal}
                            className="pointer-events-auto absolute right-4 top-4 md:relative md:top-auto md:right-auto w-8 h-8 md:w-9 md:h-9 rounded-full bg-[#181818]/80 md:bg-[#181818] text-white flex items-center justify-center hover:bg-[#333] transition cursor-pointer backdrop-blur-sm md:backdrop-blur-none"
                        >
                    ...
                    {loading || !details ? (
                        ...
                        <>
                            ...
                            {/* Mobile Info Body (Text Title, Buttons, Metadata) — Staggered with simple tweens */}
                            <motion.div
                            ...
                            >
                                <motion.h1 className="text-2xl font-bold text-white tracking-tight" variants={{ hidden: { opacity: 0, y: 12 }, visible: { opacity: 1, y: 0, transition: MOBILE_EASE } }}>{details.title || details.name}</motion.h1>

                                <motion.div className="flex items-center flex-wrap gap-3 text-gray-400 text-xs" variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: MOBILE_EASE } }}>
                                ...
                                <motion.div className="text-xs text-gray-400 space-y-1 pt-2" variants={{ hidden: { opacity: 0, y: 8 }, visible: { opacity: 1, y: 0, transition: MOBILE_EASE } }}>
